...
def get_queue_id(match_id):
    """
    Get queue id, which stands for match format (SoloDuo, Flex...)
    :param match_id:
    :return: queue id
    """
...
def calculate_team_level(match_id ,queueID=420):
    """
    Calculate team level, gather all players' level.
    :param match_id:
    :param queueID:
    :return: dict, with teams as keys, and corresponding array of values
    """
    match =get_match_by_id(match_id)
    print(match)
    WAITING_INTERVAL=2
...
def calculate_match_level(match_id,queueID=420):
    '''
    Pretty much the same as calculate_team_level, but processing it and outputting one value, meaning match level
    :param match_id:
    :param queueID:
    :return:
    '''
...
def get_literal_match_level(match_level:int):
    '''
    Function for getting literal match_level. 1000 lp -> Silver II
    :param match_level:
    :return: string, literal match level
    '''
...
def get_match_wanted_data(match_id):
    '''
    Get and process all useful data from match.
    :param match_id:
    :return: dict with wanted values
    '''
    match = get_match_by_id(match_id)
    match_info = match['info']
    time = match_info['gameDuration']
    if time<200: # if match is cancelled, don't analyze
        return None
...
def get_match_json(match_id):
    '''
    Use get_match_wanted_data and merge it with match_id and match lvl.
    :param match_id:
    :return: Dict ready for json dump.
    '''
